...
const ListingList: React.FC<ListingListProps> = ({ listings }) => {
  ...
  const removeListing = async (listingId: string) => {
    try {
      const response = await fetch(`http://34.87.41.75/staff/reported-listing/remove/${removeListing}`, {
        method: 'DELETE',
        headers: {
  ...
  const ignoreListing = async (listingId: string) => {
    try {
      const response = await fetch(`http://34.87.41.75/staff/reported-listing/ignore/${removeListing}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response.ok) {
        console.log('Listing berhasil diignore');
        setListings(prevListings => prevListings.filter(listing => listing.listingId !== listingId));
      } else {
        console.error('Gagal ignore listing:', response.statusText);
      }
    } catch (error) {
      console.error('Error:', error);
    }
  ...
  const addListing = async (listingId: string) => {
    try {
      const response = await fetch(`http://34.87.41.75/staff/reported-listing/add/${removeListing}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (response.ok) {
        console.log('Reported Listing berhasil ditambahkan');
        setListings(listings => listings);
      } else {
        console.error('Gagal menambah Reported listing:', response.statusText);
      }
    } catch (error) {
      console.error('Error:', error);
    }
                  ...
                  <div className="px-6 py-4">
                    ...
                    <div className="mt-2">
                      <div className="inline-flex items-center rounded-md shadow-sm">
                        <a
                            href={`/staff/ignore?id=${listing.listingId}`}
